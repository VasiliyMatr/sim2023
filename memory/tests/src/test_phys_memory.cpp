...
TEST_F(PhysMemoryTest, readWrite) {
    ...
    for (const auto &test_case : test_cases) {
        ...
        ASSERT_EQ(pm.write(pa, value).status, SimStatus::OK) << pa;
    }
    ...
    for (const auto &test_case : test_cases) {
        ...
        uint64_t read_64 = 0;
        ASSERT_EQ(pm.read(pa, read_64).status, SimStatus::OK) << pa;
        ASSERT_EQ(read_64, value) << pa;

        uint32_t read_32 = 0;
        ASSERT_EQ(pm.read(pa, read_32).status, SimStatus::OK) << pa;
        ASSERT_EQ(read_32, static_cast<uint32_t>(value)) << pa;

        uint16_t read_16 = 0;
        ASSERT_EQ(pm.read(pa, read_16).status, SimStatus::OK) << pa;
        ASSERT_EQ(read_16, static_cast<uint16_t>(value)) << pa;

        uint8_t read_8 = 0;
        ASSERT_EQ(pm.read(pa, read_8).status, SimStatus::OK) << pa;
...
TEST_F(PhysMemoryTest, rangeError) {
    ...
    for (const auto &pa : RANGE_ERROR_TEST_CASES) {
        ...
        ASSERT_EQ(pm.read(pa, dst).status, SimStatus::PHYS_MEM__ACCESS_FAULT)
            << pa;
        ASSERT_EQ(pm.write(pa, uint64_t{}).status,
                  SimStatus::PHYS_MEM__ACCESS_FAULT)
            << pa;
    }
}
...
TEST_F(PhysMemoryTest, pageAlignError) {
    ...
    for (const auto &pa : PAGE_ALIGN_ERROR_TEST_CASES) {
        ...
        ASSERT_EQ(pm.read(pa, dst).status,
                  SimStatus::PHYS_MEM__PAGE_ALIGN_ERROR)
            << pa;
        ASSERT_EQ(pm.write(pa, uint64_t{}).status,
                  SimStatus::PHYS_MEM__PAGE_ALIGN_ERROR)
            << pa;
...
TEST_F(PhysMemoryTest, constHostPtr) {
    ...
    ASSERT_EQ(status1, SimStatus::OK);
    ...
    ASSERT_EQ(status2, SimStatus::OK);
...
TEST_F(PhysMemoryTest, hostPtrCoherency) {
    ...
    for (size_t i = 0; i != PAGE_SIZE; ++i) {
        ASSERT_EQ(pm.write(pa + i, test_data[i]).status, SimStatus::OK) << i;
    ...
    for (size_t i = 0; i != PAGE_SIZE; ++i) {
        ...
        ASSERT_EQ(pm.read(pa + i, byte).status, SimStatus::OK) << i;
